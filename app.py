    ...
    for pair_key, ticker in pairs.items():
        ...
        with col4:
            if r_state["active"]:
                ...
                if p == 1 and p100 != 0: st.code(f"100%: {p100:.5f}")
                elif p == 2: st.code(f"0%: {p0:.5f}\n100%: {p100:.5f}")
                elif p == 3 and tgt != 0: st.code(f"ﾌﾞﾚｲｸ基準: {tgt:.5f}")
            else:
                if env_data and env_data['dir'] != "エラー":
                    ...
                    if sim_p == 1 and sim_100 != 0: st.code(f"100%: {sim_100:.5f}")
                    elif sim_p == 2 and sim_0 != 0: st.code(f"0%: {sim_0:.5f}\n100%: {sim_100:.5f}")
                else:
                    st.write("待機(停止中)")
