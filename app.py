...
with st.expander("🔔 通常・トレンドアラート管理", expanded=False):
    if st.button("🗑️ アラートデータを全消去する"):
        data["alerts"] = []
        save_data(data)
        st.success("初期化完了！")
        st.rerun()
    st.write("※アラートの追加・削除は省略しています（既存の機能のまま動作します）")
        ...
        with col4:
            if r_state["active"]:
                ...
                if env_data and env_data['dir'] != "エラー":
                    ...
                    if sim_p == 1 and sim_100 != 0: st.code(f"100%: {sim_100:.5f} ({fmt_t(t100)})")
                    elif sim_p in [2,3] and sim_0 != 0: st.code(f"0%: {sim_0:.5f} ({fmt_t(t0)})\n100%: {sim_100:.5f} ({fmt_t(t100)})")
